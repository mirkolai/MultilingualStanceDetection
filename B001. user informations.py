            ...
            if cur.fetchone()== None:
                ...
                if "id" in jsonuser:
                    ...
                    cur.execute("INSERT INTO `dictionary_mentions`(`id`, `screen_name`, `name`, `description`, `place`, `json`,language)"
                                " VALUES"
                                " (%s,%s,%s,%s,%s,%s,%s)"
                                " on duplicate key update id=id",(jsonuser['id'],jsonuser['screen_name'],jsonuser['name'],jsonuser['description'],jsonuser['location'],json.dumps(jsonuser),table["language"]))
                    db.commit()
