...
for i in range(0,len(training)):
    ...
    for key, clf in clfs.items():
        ...
        for k in range(0,len(feature_names)):
            ...
            for x in range(0,len(feature_filtered)):

                if key == "NB":
                    current_clf=copy.copy(clf)
                    X_train,X_test,feature_name_global,feature_index_global=feature_manager.create_feature_space(tweets_training,feature_filtered[:x] + feature_filtered[x+1 :],tweets_test)
                ...
                file.write('"'+key+'"'+'\t'+
                           '"'+label[i]+'"'+'\t'+
                           '"'+(' '.join(feature_filtered[:x] + feature_filtered[x+1 :]))+'"'+'\t'+
                           str(fmacro)+'\t'+
                           str(f)+'\t'+
                           str(recall)+'\t'+
                           str(prec)+'\t'+
